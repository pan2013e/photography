...
`/story-<name>` and lists every published story at `/stories`. On GitHub
...
Use a short lowercase filename with hyphens, and end it with the language it is
written in:

```text
_stories/night-trains-in-japan_en.md
_stories/night-trains-in-japan_zh.md
```

The part before the suffix is what pairs the two together, and it is what
appears in the URL: both of those are published, at `/story-night-trains-in-japan`
and `/zh/story-night-trains-in-japan`. The language switch moves between them.

A file with no suffix -- `night-trains-in-japan.md` -- is published in every
language as it stands. Use that for a story that needs no translation.

A story written in only one language is still published in all of them, with
that language's navigation around it and a small note saying it has not been
translated. So you can write in English now and add `_zh` whenever you like:
nothing else changes, and the URLs do not move.
...
- `ai_text: true` prints a small "AI-generated text" note beside the dates,
  for a story whose words were written by a language model rather than by you.
  Leave it out and nothing is shown.
...
- the story reads correctly in each language at `/stories` and `/zh/stories`,
  and the language switch in the header moves between the two tellings.
