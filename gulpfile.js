...
gulp.task('rename-images', async function () {
    for (const dir of ['main', 'aviation', 'wildlife']) {
        await gulp.src(`images_tmp/${dir}/*.*`)
            .pipe(rename(function (path) {
                var stats = fs.statSync(`images_tmp/${dir}/${path.basename}${path.extname}`);
                var timestamp = new Date(stats.mtime).getTime();
                path.basename = `${dir}-${timestamp}`;
            }))
